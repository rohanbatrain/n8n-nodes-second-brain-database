...
import { adminOperations, adminFields } from './AdminDescription';
import { ipamOperations, ipamFields } from './IpamDescription';
import { blogOperations, blogFields } from './BlogDescription';
import { profileOperations, profileFields } from './ProfileDescription';
import { assetsOperations, assetsFields } from './AssetsDescription';
import { clubsOperations, clubsFields } from './ClubsDescription';
import { skillsOperations, skillsFields } from './SkillsDescription';
import { documentsOperations, documentsFields } from './DocumentsDescription';
			...
			// AI Operations
			{
				...
				options: [
					...
					},
					{
						...
						action: 'Delete session an ai',
					},
					{
						name: 'Update Session',
						value: 'updateSession',
						description: 'Update an AI session',
						action: 'Update session an ai',
					},
					{
						name: 'End Session',
						value: 'endSession',
					...
					case 'ai':
						switch (operation) {
							...
							case 'setupVoiceSession': {
								...
								responseData = await this.helpers.httpRequest({
									method: 'POST',
									url: `/chat/sessions/${sessionId}/voice/setup`,
									json: true,
								});
								break;
							}
							case 'processVoiceInput': {
								...
								responseData = await this.helpers.httpRequest({
									method: 'POST',
									url: `/chat/sessions/${sessionId}/voice/process`,
									body,
									json: true,
								});
								break;
							...
							case 'getVoiceToken': {
								...
								responseData = await this.helpers.httpRequest({
									method: 'GET',
									url: `/chat/sessions/${sessionId}/voice/token`,
									json: true,
								});
								break;
							}
							case 'getSessionPerformance': {
								...
								responseData = await this.helpers.httpRequest({
									method: 'GET',
									url: `/chat/sessions/${sessionId}/performance`,
									json: true,
								});
								break;
							}
						...
						switch (operation) {
							case 'uploadDocument': {
								...
								responseData = await this.helpers.httpRequest({
									...
									body: formData,
									// json: true, // n8n handles multipart automatically if body contains file
								});
								break;
							}
							case 'query': {
								...
								const body: IDataObject = { query: queryText };
							...
							default:
								throw new NodeOperationError(this.getNode(), `Unknown RAG operation: ${operation}`);
						}
						break;
					case 'admin':
						switch (operation) {
							...
							case 'listBlocklist':
								...
								break;
							case 'listAbuseEvents': {
								const qs: IDataObject = {};
								const filterEmail = this.getNodeParameter('filterEmail', itemIndex) as string;
								...
								if (filterEmail) qs.email = filterEmail;
								if (filterType) qs.type = filterType;
								if (resolved !== undefined) qs.resolved = resolved;
								responseData = await this.helpers.httpRequest({
									method: 'GET',
							...
							case 'resolveAbuseEvent': {
								...
								responseData = await this.helpers.httpRequest({
									method: 'POST',
									url: '/admin/abuse-events/resolve',
							...
							default:
								throw new NodeOperationError(this.getNode(), `Unknown Admin operation: ${operation}`);
						}
						break;
					case 'ipam':
						switch (operation) {
							...
							case 'getCountry': {
								...
								responseData = await this.helpers.httpRequest({
									method: 'GET',
									url: `/ipam/countries/${country}`,
									json: true,
								});
								break;
							}
							case 'getCountryUtilization': {
								const country = this.getNodeParameter('country', itemIndex) as string;
								responseData = await this.helpers.httpRequest({
									method: 'GET',
									url: `/ipam/countries/${country}/utilization`,
									json: true,
								});
								break;
							}
							case 'createRegion': {
								const country = this.getNodeParameter('country', itemIndex) as string;
								const name = this.getNodeParameter('regionName', itemIndex) as string;
								const description = this.getNodeParameter('description', itemIndex) as string;
								const tags = this.getNodeParameter('tags', itemIndex, {}) as IDataObject;
								responseData = await this.helpers.httpRequest({
									method: 'POST',
									url: '/ipam/regions',
									body: { country, name, description, tags },
									json: true,
								});
								break;
							}
							case 'listRegions': {
								const qs: IDataObject = {};
								const filterCountry = this.getNodeParameter('filterCountry', itemIndex) as string;
								const filterStatus = this.getNodeParameter('filterStatus', itemIndex) as string;
								if (filterCountry) qs.country = filterCountry;
							...
							case 'getRegion': {
								const regionId = this.getNodeParameter('regionId', itemIndex) as string;
								responseData = await this.helpers.httpRequest({
									method: 'GET',
									url: `/ipam/regions/${regionId}`,
									json: true,
								});
								break;
							}
							...
							case 'getTopCountries':
								responseData = await this.helpers.httpRequest({
									method: 'GET',
									url: '/ipam/dashboard/top-countries',
									json: true,
								});
								break;
							case 'getRecentActivity':
								responseData = await this.helpers.httpRequest({
									method: 'GET',
									url: '/ipam/dashboard/recent-activity',
									json: true,
								});
								break;
							default:
								throw new NodeOperationError(this.getNode(), `Unknown IPAM operation: ${operation}`);
						}
						break;
					case 'blog':
						switch (operation) {
							...
							case 'createPost': {
								...
								responseData = await this.helpers.httpRequest({
									method: 'POST',
									url: `/blog/websites/${websiteId}/posts`,
									body: { title, content },
									json: true,
								});
								break;
							...
							case 'getPost': {
								const websiteId = this.getNodeParameter('websiteId', itemIndex) as string;
								const postSlug = this.getNodeParameter('postSlug', itemIndex) as string;
								responseData = await this.helpers.httpRequest({
									method: 'GET',
									url: `/blog/websites/${websiteId}/posts/${postSlug}`,
									json: true,
								});
								break;
							}
							case 'updatePost': {
								...
								responseData = await this.helpers.httpRequest({
									method: 'PUT',
									url: `/blog/websites/${websiteId}/posts/${postId}`,
									body: { title, content },
									json: true,
								});
								break;
							...
							default:
								throw new NodeOperationError(this.getNode(), `Unknown Blog operation: ${operation}`);
						}
						break;
					case 'profile':
						switch (operation) {
							...
							default:
								throw new NodeOperationError(this.getNode(), `Unknown Profile operation: ${operation}`);
						}
						break;
					case 'assets':
						switch (operation) {
							...
							case 'getBanner':
								...
								break;
							default:
								throw new NodeOperationError(this.getNode(), `Unknown Assets operation: ${operation}`);
						}
						break;
					case 'clubs':
						// https://github.com/rohanbatrain/second_brain_database/blob/main/src/second_brain_database/routes/clubs.py
						if (operation === 'createUniversity') {
							const name = this.getNodeParameter('name', itemIndex) as string;
							const domain = this.getNodeParameter('domain', itemIndex) as string;
							const additionalFields = this.getNodeParameter('additionalFields', itemIndex, {}) as IDataObject;
							const body = {
							...
							responseData = await this.helpers.httpRequest({
								method: 'POST',
								url: '/clubs/universities',
								body,
								json: true,
							});
						} else if (operation === 'getUniversity') {
							const universityId = this.getNodeParameter('universityId', itemIndex) as string;
							responseData = await this.helpers.httpRequest({
								method: 'GET',
								url: `/clubs/universities/${universityId}`,
								json: true,
							});
						} else if (operation === 'listUniversities') {
						...
						} else if (operation === 'deactivateClub') {
							const clubId = this.getNodeParameter('clubId', itemIndex) as string;
							responseData = await this.helpers.httpRequest({
								method: 'DELETE',
								url: `/clubs/${clubId}`,
								json: true,
							});
						} else if (operation === 'getClub') {
							const clubId = this.getNodeParameter('clubId', itemIndex) as string;
							responseData = await this.helpers.httpRequest({
								method: 'GET',
								url: `/clubs/${clubId}`,
						...
						} else if (operation === 'listClubs') {
							const universityId = this.getNodeParameter('universityId', itemIndex) as string;
							const category = this.getNodeParameter('category', itemIndex) as string;
							const search = this.getNodeParameter('search', itemIndex) as string;
							const qs: IDataObject = {};
							if (universityId) qs.university_id = universityId;
						...
						} else if (operation === 'createVertical') {
							const clubId = this.getNodeParameter('clubId', itemIndex) as string;
							const name = this.getNodeParameter('name', itemIndex) as string;
							const additionalFields = this.getNodeParameter('additionalFields', itemIndex, {}) as IDataObject;
							const body = {
							...
							responseData = await this.helpers.httpRequest({
								method: 'POST',
								url: `/clubs/${clubId}/verticals`,
								body,
								json: true,
							});
						} else if (operation === 'getVertical') {
							const verticalId = this.getNodeParameter('verticalId', itemIndex) as string;
							responseData = await this.helpers.httpRequest({
								method: 'GET',
								url: `/clubs/verticals/${verticalId}`,
								json: true,
							});
						} else if (operation === 'listVerticals') {
							const clubId = this.getNodeParameter('clubId', itemIndex) as string;
							responseData = await this.helpers.httpRequest({
								method: 'GET',
								url: `/clubs/${clubId}/verticals`,
						...
						} else if (operation === 'updateVertical') {
							const verticalId = this.getNodeParameter('verticalId', itemIndex) as string;
							const additionalFields = this.getNodeParameter('additionalFields', itemIndex, {}) as IDataObject;
							const body = {
								...additionalFields,
							...
							responseData = await this.helpers.httpRequest({
								method: 'POST',
								url: `/clubs/${clubId}/members/invite`,
								body,
								json: true,
							});
						} else if (operation === 'updateMemberRole') {
							...
							responseData = await this.helpers.httpRequest({
								method: 'PUT',
								url: `/clubs/members/${memberId}/role`,
								body,
								json: true,
							});
						}
						break;
					case 'skills':
						// https://github.com/rohanbatrain/second_brain_database/blob/main/src/second_brain_database/routes/skills.py
						if (operation === 'createSkill') {
						...
						} else if (operation === 'deleteSkill') {
							const skillId = this.getNodeParameter('skillId', itemIndex) as string;
							responseData = await this.helpers.httpRequest({
								method: 'DELETE',
								url: `/api/v1/skills/${skillId}`,
								json: true,
							});
						} else if (operation === 'getSkill') {
							const skillId = this.getNodeParameter('skillId', itemIndex) as string;
							const includeAnalytics = this.getNodeParameter('includeAnalytics', itemIndex) as boolean;
							const qs: IDataObject = {
						...
						} else if (operation === 'getSkillTree') {
							responseData = await this.helpers.httpRequest({
								method: 'GET',
								url: '/api/v1/skills/tree',
								json: true,
							});
						} else if (operation === 'linkParentSkill') {
							const skillId = this.getNodeParameter('skillId', itemIndex) as string;
							const parentId = this.getNodeParameter('parentId', itemIndex) as string;
							responseData = await this.helpers.httpRequest({
								method: 'POST',
								url: `/api/v1/skills/${skillId}/link/${parentId}`,
								json: true,
							});
						} else if (operation === 'listSkills') {
							const includeAnalytics = this.getNodeParameter('includeAnalytics', itemIndex) as boolean;
						...
						} else if (operation === 'unlinkParentSkill') {
							const skillId = this.getNodeParameter('skillId', itemIndex) as string;
							const parentId = this.getNodeParameter('parentId', itemIndex) as string;
							responseData = await this.helpers.httpRequest({
								method: 'DELETE',
						...
						} else if (operation === 'addSkillLog') {
							const skillId = this.getNodeParameter('skillId', itemIndex) as string;
							const projectId = this.getNodeParameter('projectId', itemIndex) as string;
							const progressState = this.getNodeParameter('progressState', itemIndex) as string;
							const numericLevel = this.getNodeParameter('numericLevel', itemIndex) as number;
							const notes = this.getNodeParameter('notes', itemIndex) as string;
							const context = this.getNodeParameter('context', itemIndex, {}) as IDataObject;
							const body: IDataObject = {
							...
							responseData = await this.helpers.httpRequest({
								method: 'POST',
								url: `/api/v1/skills/${skillId}/logs`,
								body,
								json: true,
							});
						} else if (operation === 'getSkillLogs') {
							const skillId = this.getNodeParameter('skillId', itemIndex) as string;
							const limit = this.getNodeParameter('limit', itemIndex) as number;
							const skip = this.getNodeParameter('skip', itemIndex) as number;
							const qs: IDataObject = {
						...
						break;
					case 'documents':
						// https://github.com/rohanbatrain/second_brain_database/blob/main/src/second_brain_database/routes/documents.py
						if (operation === 'chunkDocument') {
							const documentId = this.getNodeParameter('documentId', itemIndex) as string;
							...
							responseData = await this.helpers.httpRequest({
								method: 'POST',
								url: `/documents/${documentId}/chunk`,
								qs,
								json: true,
							});
						...
						} else if (operation === 'extractTables') {
							const binaryPropertyName = this.getNodeParameter('binaryPropertyName', itemIndex) as string;
						...
						} else if (operation === 'getProcessingStatus') {
							const taskId = this.getNodeParameter('taskId', itemIndex) as string;
							responseData = await this.helpers.httpRequest({
								method: 'GET',
								url: `/documents/status/${taskId}`,
								json: true,
							});
							...
							const formData = {
								...
								extract_images: extractImages.toString(),
